...
describe("trader authentication wiring", () => {
  it("never names the internal audience's login route", () => {
    // UI-ISO-001. The route is the audience (DOC-CONFLICT-023), so naming the
    // other one is the only way this bundle could ask to be evaluated as an
    // internal caller.
    for (const file of [["src", "auth.ts"], ["app", "login", "page.tsx"]]) {
      expect(source(...file)).not.toContain("/auth/admin/login");
      expect(source(...file)).not.toContain("gp_admin_session");
      expect(source(...file)).not.toContain("gp_admin_csrf");
    }
  });

  it("names its own login route, so the check above is not vacuous", () => {
    expect(source("src", "auth.ts")).toContain("/auth/trader/login");
  });
  ...
  it("reads the CSRF token from its own cookie and no other", () => {
    const jar =
      "other=1; __Host-gp_admin_csrf=admin-token; __Host-gp_trader_csrf=trader-token; x=2";

    expect(readCsrfToken(jar)).toBe("trader-token");
    expect(readCsrfToken("__Host-gp_admin_csrf=admin-token")).toBeUndefined();
    expect(readCsrfToken("")).toBeUndefined();
  });

  it("does not mistake a cookie whose name merely ends the same way", () => {
    // `x__Host-gp_trader_csrf` is a different cookie. Splitting on `=` after
    // trimming compares the whole name, which a `includes()` check would not.
    expect(readCsrfToken("x__Host-gp_trader_csrf=wrong")).toBeUndefined();
  });
});
