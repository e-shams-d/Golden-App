...
Status: **M1 ACCEPTED — تصویب مالک در ۲۰۲۶-۰۸-۰۱ (بخش ۸)**
...
**درباره‌ی رابطه‌ی این فایل با `TRANSFER_SHA`:** این فرم به کامیتی اشاره می‌کند
که شواهد روی آن تولید شده، و خودش در یک کامیت *بعدی* ثبت می‌شود؛ هیچ کامیتی
نمی‌تواند SHA خودش را در محتوای خود بنویسد (قالب رسمی هم همین را الزام
می‌کند). پس طبیعی است که `git log` یک کامیت evidence-only بالاتر از
`TRANSFER_SHA` نشان دهد. آنچه باید همیشه برقرار باشد این است: SHA ثبت‌شده در
جدول زیر باید همان کامیتی باشد که هر دو verifier روی آن اجرا شده‌اند و همان
که به remote رفته است. اگر بعد از این کامیت، تغییری در **کد یا زیرساخت** وارد
شود، پذیرش باید تکرار و این جدول به‌روز شود.
...
| نام remote | `origin` |
| شاخه‌ی تحویل | `main` |
| SHA مورد انتظار (TRANSFER_SHA) | `1541f14e94342ce049e7836d61e02445e632ff4c` |
| SHA دریافت‌شده از remote | `1541f14e94342ce049e7836d61e02445e632ff4c` — push مالک در ۲۰۲۶-۰۷-۲۸ ساعت ۲۳:۴۱ (+۰۳:۳۰) |
| SHA در clone تمیز | `1541f14e94342ce049e7836d61e02445e632ff4c` — هر دو verifier روی همین SHA با درخت تمیز اجرا و پاس شدند (۲۰۲۶-۰۸-۰۱) |
...
اجرا روی `1541f14` (همان SHA منتشرشده روی remote) با
`Native M1 verification passed.` و drift صفر. شمارش تست‌ها در تمام اجراها
یکسان ماند: ۴۴ تست بک‌اند، ۱۸ تست واحد فرانت‌اند، ۱۲ تست دسترسی‌پذیری.
Log artifacts (خارج از Git): `.local/m1-evidence/verify-native-1541f14-2026-08-01.log`
(نهایی) و اجراهای قبلی `verify-native-2026-07-25.log`،
...
«automated Docker gates passed». آخرین اجرا روی `1541f14` (همان SHA منتشرشده)
با `M1_VERIFY_HTTP_PORT=18081`، چون پورت پیش‌فرض ۱۸۰۸۰ را یک کانتینر پروژه‌ی
دیگر روی همان ماشین اشغال کرده بود؛ پورت از پارامترهای پشتیبانی‌شده‌ی خود
verifier است و ایزوله بودن ingress روی loopback تغییری نکرد.
Log artifacts (خارج از Git): `.local/m1-evidence/verify-docker-1541f14-2026-08-01.log`
(نهایی) و اجراهای قبلی `verify-docker-2026-07-25.log`،
...
موارد زیر هنگام پذیرش **باز بوده‌اند و آگاهانه پذیرفته شده‌اند**؛ هیچ‌کدام فنی
نیستند و هیچ‌کدام مانع کارهای مجاز پس از M1 نمی‌شوند، اما هرکدام پیش از
رویداد ذکرشده در ستون «موعد» باید بسته شوند:

| مورد باز | شدت | مالک | موعد | شرط بستن |
|---|---|---|---|---|
| تصویب سیاست release (سیاست شاخه و بازبینی عملاً اعمال شده) | متوسط | مالک/حاکمیت | پیش از اولین انتشار | ثبت مکتوب سیاست انتشار |
...
تصویب‌کننده: **Ehsan Shams — مالک پروژه**
تاریخ: **2026-08-01**
مرجع تصمیم: تصویب مالک در جلسه‌ی کاری پذیرش؛ نام حقوقی/سمت سازمانی ثبت نشده
(همان قراردادی که `docs/governance/M0_READINESS.md` برای تصویب گفت‌وگویی به کار
می‌برد). ثبت این تصویب در مخزن با دستور صریح مالک انجام شده است.

### دامنه‌ی این تصویب

این امضا **فقط** پذیرش M1 را تأیید می‌کند: مخزن، runtime، تکرارپذیری روی
سیستم مقصد، و گیت‌های تحویل فهرست‌شده در بخش‌های ۱ تا ۷ با شواهدشان. این امضا
شامل موارد زیر **نیست** و هیچ‌کدام را تصویب نمی‌کند:

- بسته‌ی اسناد منبع، catalogueهای machine-readable، یا هر تصمیم باز M0
- انتشار تولیدی یا پذیرش پیاده‌سازی
- موارد باز جدول بالا، که آگاهانه باز پذیرفته شده‌اند

**مرزهای M0 همچنان کاملاً برقرارند.** طبق `docs/governance/M0_READINESS.md` و
بند ۱۴ سند تحویل، ۲۶ conflict باز (۵ مورد Critical) و ۱۵ گیت باز از ۲۴ باقی
است. پذیرش M1 هیچ‌یک از آن‌ها را باز نمی‌کند: کار مالی — migration، enum،
command/API، workflow معامله یا پرداخت، authorization بر پایه‌ی permission
freezeنشده، و UI مالی — تا تصویب قرارداد مربوطه ممنوع می‌ماند. آنچه پس از این
پذیرش مجاز است، همان چیزی است که بند ۱۴ اجازه می‌دهد: foundationهای برگشت‌پذیر،
tooling، observability، shellهای UI، adapterهای بدون منطق مالی، و تست زیرساخت.
