...
| ID | Question | Blocks |
|---|---|---|
| **G-1** | ~~Which Excel writer, and is it acceptable on an air-gapped deployment?~~ **ANSWERED 2026-08-22: `openpyxl==3.1.5`, approved by the project owner.** The open part was never technical — it was whether a new production dependency is acceptable on a network that cannot reach a registry, which is a deployment fact only the owner holds. What was pinned after the answer, and only after being measured rather than assumed: MIT licensed, **pure Python** (no `.so` anywhere in the installed tree for it or for its single dependency `et-xmlfile`), so no compiler is needed on the target and two wheels can be vendored. All of §15.6's properties were verified against a file written and read back *before* the pin — amounts exact to 12,345,678,901,234 as `int`, Persian text intact, row order preserved, formula-like text renderable inert — and each is re-asserted by a test rather than left to this note. **One thing the measurement changed:** openpyxl stores an apostrophe prefix *inside the cell value*, so a bank opening the file sees `'=HYPERLINK(…)` including the quote. Not a defect, but it makes G-6's trade-off concrete rather than theoretical, and slice 2 records which escaping it chose and what a payee name then looks like. | ~~Slice 2~~ — **unblocked** |
...
| **G-13** | **What does `If-Match` target on mark-sent?** `05_API_Specification.md:1519` shows `If-Match: "rv-5"` on the request, and §11.8 gives `bank_excel_exports` **no `record_version` column** — the table is nearly immutable by design, and slice 4 grants UPDATE on four columns and no more. `command_catalog.yaml:203` says the same thing in its own words: `"concurrency": "open_conflict_if_match_target_not_defined"`, and the row's status is `blocked_by_if_match_contract`. So the catalogue is describing an unresolved contract rather than a requirement being ignored. Three possible answers: the header targets the **batch's** `record_version` (which is what finalize's `If-Match` does, and would make a stale batch read refuse a sending); the table gains a `record_version`, which document 04 would owe; or the header is an editorial error and `Idempotency-Key` alone is the contract. Slice 4 requires the key, refuses a second sending with `409`, and does not invent a target. | Nothing in M7; the key already makes a retry safe |

**G-1 was answered on 2026-08-22 and nothing now blocks the milestone.** While it was open, slices
1, 5A and 6A shipped — every part of M7 that touches no file — which is why the split-slice pattern
in this plan exists at all. None of G-2 through G-13 blocks starting; each records a decision this
plan made and names what would change if the owner decides otherwise.
